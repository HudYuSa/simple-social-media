...
type CommentController interface {
	...
	GetCommentReplies(ctx *gin.Context)
...
func (cc *commentController) GetPostComments(ctx *gin.Context) {
	...
	result := cc.DB.Where("parent_comment_id IS NULL AND post_id = ?", postID).Offset((pageOffset - 1) * 5).Limit(5).Order("created_at DESC").Find(&comments)
	if result.Error != nil {
	...
	for _, comment := range comments {
		...
		result := cc.DB.Where("parent_comment_id = ?", comment.ID).Find(&models.Comment{}).Count(&replyCount)
		if result.Error != nil {
			switch result.Error.Error() {
			default:
				dtos.RespondWithError(ctx, http.StatusBadGateway, result.Error.Error())
...
func (cc *commentController) GetCommentReplies(ctx *gin.Context) {
	...
	result := cc.DB.Where("parent_comment_id = ?", commentID).Offset((pageOffset - 1) * 5).Limit(5).Order("created_at DESC").Find(&comments)
	if result.Error != nil {
		switch result.Error.Error() {
		default:
			dtos.RespondWithError(ctx, http.StatusBadGateway, result.Error.Error())
